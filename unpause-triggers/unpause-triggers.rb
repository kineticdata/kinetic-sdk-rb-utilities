...
# Get Paused Triggers
response =  conn.get("#{config['SPACE_URL']}/app/components/task/app/api/v2/triggers/paused", parameters, conn.default_headers)
if response.content['triggers'].length > 0
	puts "#{response.content['triggers'].length} paused triggers were found"
	puts "Would you like to unpause them? (Y/N)"
	STDOUT.flush
	case (gets.downcase.chomp)
	when 'y'
	  puts "Unpausing Triggers"
	  STDOUT.flush
	else
	  abort "Exiting"
	end
	...
	response.content['triggers'].each{ |trigger| 
		conn.put("#{config['SPACE_URL']}/app/components/task/app/api/v2/triggers/#{trigger['id']}", {"status": "Unpaused"}, conn.default_headers)
	}
else
