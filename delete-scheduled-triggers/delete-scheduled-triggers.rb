...
# Get Paused Triggers
response =  conn.get("#{config['SPACE_URL']}/app/components/task/app/api/v2/triggers/scheduled", parameters, conn.default_headers)
if response.content['triggers'].length > 0
	puts "#{response.content['triggers'].length} scheduled triggers were found"
